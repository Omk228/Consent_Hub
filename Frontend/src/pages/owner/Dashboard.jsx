...
const OwnerDashboard = () => {
  ...
  const [approvedRequests, setApprovedRequests] = useState([]); // Approved requests ke liye naya state
  ...
  const handleAction = async (requestId, action) => {
    ...
    try {
      ...
      setAllRequests(prev => prev.filter(r => r.id !== requestId)); // All requests se bhi hatana
  ...
  // Approved requests ko filter karna allRequests se
  useEffect(() => {
    setApprovedRequests(allRequests.filter(req => req.status === 'APPROVED'));
  }, [allRequests]); // allRequests jab change ho tabhi filter ho
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Approved Access Requests Table - Naya Section */}
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mt-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              <CheckCircle className="h-5 w-5 text-emerald-500" /> Approved Requests
            </h3>
            <Button 
              onClick={() => navigate('/owner/access-history')}
              variant="ghost"
              size="sm"
              className="text-indigo-600 hover:bg-indigo-50"
            >
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {loadingAllRequests ? (
                  ...
                  <tr><td colSpan="3" className="py-10 text-center text-red-500">{errorAllRequests}</td></tr>
                ) : approvedRequests.length === 0 ? (
                  <tr><td colSpan="3" className="py-10 text-center text-slate-400">No approved requests</td></tr>
                ) : (
                  approvedRequests.map((request) => (
                    <tr key={request.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right space-x-2">
                        ...
                        >
                          {actionLoading === request.id ? <Loader2 className="w-4 h-4 animate-spin" /> : "Revoke"}
                        </Button>
                      </td>
                    </tr>
                  ))
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          ...
          <ul className="space-y-6">
            {loadingActivities ? (
              ...
              activityLogs.slice(0, 6).map((activity) => ( // Top 6 logs dikhayेंगे dashboard pe
